...
class GenerateGaussian():
    ...
    def __init__(self, seed = 1, n_obs = 20, x_lim = (0, 10), y_lim = (0, 10), T = 400, n_lags = 3, weight = [0.5, 0.25, 0.15, 0.1], dim=2, var=1, len_scale=2):
    ...
    def weighted_average(self, field, weights, start, end):
        return [sum(weights[j] * field[i-j] for j in range(len(weights))) for i in range(start, end)]
    ...
    def generate_field(self):
        ...
        self.field = np.array([srf.structured([self.x_field, self.y_field], seed=i) for i in range(end_idx)])
        self.smooth_field = np.array(self.weighted_average(self.field, self.weight, start_idx, end_idx))


    def sample_locations(self):
        random.seed(self.seed)
        self.x_idxs = random.choices(range(len(self.x_field)), k=self.n_obs) # with replacement
        self.y_idxs = random.sample(range(len(self.y_field)), k=self.n_obs) # without replacement


    def get_dataframe(self):
        return pd.DataFrame([[t, i, self.smooth_field[t][x_idx][y_idx]]
    ...
    def field_plot(self, n_times=3, figsize=(10,7)):
        ...
        for i in range(n_times):
            ...
            ax[i].text(3, 2, 't={}'.format(i+1), fontsize=18, c='white')
...
class GenerateAnomaly():
    ...
    def drift(self, size):
        return np.arange(self.drift_delta, (size+1)*self.drift_delta, self.drift_delta)
    ...
    def generate_anomaly(self, anomaly_func, lam = 3, prop_anom = 0.07, seed=45):
        ...
        for i in range(num_anom):
            ...
            # update dataframe
            self._X.loc[idx_start:idx_end, col] += anomaly_func(size=idx_end - idx_start + 1)

            # keep record of anomaly type, and indices
            if col in self.ANOMS[anomaly_func.__name__].keys():
                self.ANOMS[anomaly_func.__name__][col] = np.concatenate((self.ANOMS[anomaly_func.__name__][col], np.arange(idx_start, idx_end+1)))
            else:
                self.ANOMS[anomaly_func.__name__][col] = np.arange(idx_start, idx_end+1)
